...
- BoatraceCSV の `programs/title` + `programs/race_cards` + `previews/stt` + `index` を入力
- 強さpt（5要素の寄与pt）と進入コース・スタート展示を踏まえた展開シナリオ分析
- 構造化された展開予想テキスト（JSON）を出力

> **注**: 旧 proposal では Prediction Previews + Estimates の ML 予測結果（LambdaRank + Random Forest / LightGBM）を入力する想定だったが、これらの CSV は廃止済み。
...
| CSV種別 | パス | AI パイプラインでの用途 | AM 9:00 JST時点 |
|---|---|---|---|
| Programs (Title) | `data/programs/title/YYYY/MM/DD.csv` | レース名・グレード・締切時刻などのメタ情報 → ヘッダ生成 | 取得可能 |
| Programs (Race Cards) | `data/programs/race_cards/YYYY/MM/DD.csv` | 出走表データ（選手・モーター・成績） → 予想分析の入力 | 取得可能 |
| Previews (STT) | `data/previews/stt/YYYY/MM/DD.csv` | 直前情報（進入コース・スタート展示） → スタート隊形の推定 | 締切 5 分前以降に順次公開 |
| Index | `data/index/YYYY/MM/DD.csv` | 強さpt（5要素の寄与pt） → 既存スコアを Gemini 解説の入力に | 取得可能（state=daily） |
| Results | `data/results/YYYY/MM/DD.csv` | 前日結果 → 的中実績・傾向分析 | 前日分取得可能 |

> **注**: 旧 `prediction-preview` / `estimate` / `confirm` は BoatraceCSV 上流での生成停止に伴い廃止済み。本提案の Vertex AI パイプラインを再導入する場合、ML 予測は入力として使えないため、`index` の強さpt や `programs/race_cards` の選手成績を主要根拠として Gemini に渡す構成へ調整する必要がある。

### 2.3 既存スコアの活用

旧 proposal では BoatraceCSV の ML 予測（scikit-learn ベース）を Gemini 3 Pro の入力として活用する想定だったが、現在は BoatraceCSV 側で `index` CSV による「強さpt」（5要素の寄与pt: 枠番 / 選手 / モーター / 展示 / 気象）が提供されている。Vertex AI 再導入時は、この強さpt と寄与pt を入力として Gemini に解説を組み立てさせる構成が現実的。

| BoatraceCSV 提供スコア | アルゴリズム | Gemini 3 Pro での活用（将来） |
|---|---|---|
| 強さpt | 5 要素の寄与pt 合算 | 各艇の総合力比較を解説 |
| 寄与pt（枠番/選手/モーター/展示/気象） | 重み付き分解 | 注目要素を抽出して展開シナリオを構成 |
| 進入コース・スタート展示（previews/stt） | 実測値 | スタート隊形の推定 |

> **注**: 旧 ML 予測（着順 LambdaRank + Random Forest、決まり手 LightGBM、進入コース ハンガリアンアルゴリズム、ST 回帰モデル）は CSV 廃止により利用不可。同等の予測機能を再現するには独自に推論パイプラインを構築する必要がある。
...
│   入力: programs/title + race_cards + stt + index   │
│   出力: 構造化展開予想テキスト (JSON)                │
└──────────────────┬───────────────────────────────┘
                   │
                   ▼
...
## 出走表データ (programs/race_cards CSV)
{出走表CSV: 枠番, 選手名, 級別, 全国勝率, 当地勝率, モーター2連率, 全国平均ST...}

## 直前情報 (previews/stt CSV) ※取得できたレースのみ
{各艇の進入コース, スタート展示}

## AI 総合評価 (index CSV)
{各艇の強さpt, 寄与pt（枠番/選手/モーター/展示/気象）, 状態（daily/realtime）}
...
### Phase 1: MVP（ハッカソン提出用）※未実装

- BoatraceCSV からデータ取得（`programs/title` + `programs/race_cards` + `previews/stt` + `index`）
...
- 的中実績ページ（過去 RacePrediction × `results` の自前突合）
- OGP画像対応
...
> **注**: 現実装では Vertex AI / Gemini を採用せず、BoatraceCSV の `index` CSV が提供する強さpt をそのまま AI 総合評価として静的サイトに表示している。本ロードマップは Vertex AI を再導入する場合の構想。旧 proposal の Confirmations CSV ベースの的中実績は CSV 廃止により利用不可なため、自前で過去予想 vs 結果を突合するロジックが必要。
...
2. **BoatraceCSV のみ** — GitHub Pages 配信の CSV データのみを使用。`index` CSV による強さpt（5要素の寄与pt）が AI 総合評価として利用可能で、外部 API 依存なし。旧 ML 予測 (Estimates / Prediction Previews) は廃止済み
