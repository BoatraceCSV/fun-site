...
| `SessionResultsGrid.astro` | 節間成績（今節 14 スロット）の可視化。`RaceRacer.sessionResults`（未出走除外）の着順トークンを色付きバッジで時系列に並べる。バッジの背景色は対象レースでの枠番の艇番カラー（`BOAT_COLORS`: 1=白 / 2=黒 / 3=赤 / 4=青 / 5=黄 / 6=緑、枠番不明=灰）。各バッジの `title` に 日次・走・R・進入・枠・ST を出す。`RacerTable` の展開行内で利用 |
...
| `RecentFormSection.astro` | 近況5節セクション。`RacePrediction.recentForm` から艇別に全国・当地の直近5節を表示。各節はグレード・場名・期間とともに着順を `tokenizeRankString` で色付きマーク化（1着=青/2着=緑/3着=黄/4-6着=灰、F=赤/L=橙/その他特殊=灰、優勝戦は赤枠、日区切りは空き）。節は古→新（左→右）に並べ替えて表示。`recentForm` が無いレースでは詳細ページ側で非表示 |
| `PredictorCard.astro` | 1 予想者ぶんの予想カード。表示名・買い目 (BettingPicks) ・回収率 (BetPayoutSummary)・AI 評価チャートを 1 セクションに集約。レース詳細ページは `prediction.predictions[]` をループしてこれを縦並びレンダリングする。任意 prop `startPrediction` / `oneMarkAiEvaluation` が両方渡されたとき（本命予想=slot1 / モーター予想=slot4 のカード。過去日は退役済みの slot2/3 も対象）スタート予想・1マーク予想の 2 図をカード内に 2 カラムグリッドで小さく横並び表示する。`startNote` prop は `StartPredictionDiagram` の `stNote` へ転送。任意 prop `recipeNote` を渡すとカード見出し直下に注記を表示する（モーター予想カードは motor4 差し替えを説明する。過去日カードはモーター評価変更予想=motor2rate 差し替え / 展開予想=展開優位pt 追加を説明）。スタート予想は全予想者共通で全国平均ST、1マークは予想者ごとの AI 評価を渡す（モーター予想は本命予想と recipe が異なるため図は異なり得る） |
| `BettingPicks.astro` | 当日買い目・直前買い目の三連単フォーメーションと的中可否 (PredictorCard 内部で利用)。`predictorName` prop が渡されると見出しに「{予想者名}当日買い目」「{予想者名}直前買い目」として予想者名をプレフィクスする。`predictorId` prop で買い目しきい値（`bettingToleranceFor`。現状オーバーライド無しで全予想者 ±0.10）と走行距離の予測 ST 種別（`oneMarkDistanceOptionsFor`。`useEstimatedST` な予想者 v5_slit / v7_aggregate は AI 推定 ST、他は全国平均 ST）を予想者ごとに切替。どちらもバッチの買い目生成（`prediction-builder.ts`）と同じヘルパーで解決するため、表示される買い目と的中・回収率の集計対象になる買い目が一致する。説明文のしきい値表記・予測 ST 種別も実値に追従する |
...
| `DailyBetSummary.astro` | トップページの当日サマリー。締切済み全レースを集計した 3連単 戦略の的中率・回収率を予想者別（直前買い目のみ）に表示（本命直前 / モーター直前 の 2 カード。`activePredictors()` を slot 昇順でループするため active 予想者の増減に自動追従。新スキーマ `prediction.predictions[]` を優先し、無い場合のみ primary 予想者を legacy `prediction.betPayout.realtime` でフォールバック） |
...
| `RaceResultSection.astro` | レース結果（着順・ST・決まり手・天候）。`predictorName` prop が渡されると「{予想者名}当日買い目 的中」「{予想者名}直前買い目 的中」の的中バッジも表示 |
