...
| `StartResultDiagram.astro` | スタート結果図（確定結果の進入コース順に並べた SVG）。`RaceResultSection` の「スタート（進入順）」で使う。**表示レンジ・viewBox・レーン高・スタートライン・目盛りは `StartPredictionDiagram` と同一定数**にしてあり、スタート予想と同じ目盛りで見比べられることがこの図の目的。実測値に合わせてレンジを可変にはしない（変えると予想図と比較できなくなる）。艇の円の中心は水面内 (`TRACK_LEFT + r` 〜 `TRACK_RIGHT - r`) にクランプするので、レンジ外 (-0.05〜0.30秒 の外) の艇は端で止まる。正確な値は右の数値ラベルに必ず出し、クランプが起きた回だけ凡例に断りを追記する。フライングは艇を赤縁 (stroke 2.5) + ST ラベルを赤太字の `F.03` 表記（`SessionResultsGrid` と同じ書き方で負号は出さない）。ST が 0（欠場・エンスト等で CSV 空欄）の艇は位置が決まらないので、レーン左端に `opacity 0.4` で置き数値は `—` にする |
