...
| `AiEvaluationChart.astro` | AI 総合評価（枠別 寄与pt を横棒で積み上げ。採用成分は `evaluation.componentKeys` で動的に決まる)。任意 prop `racerPtHref` を渡すと**凡例の「選手」だけがリンク**になり、選手詳細ページ（選手pt の計算根拠）へ飛ぶ。他の成分は計算過程を出せるだけの入力データを fun-site 側に持っていないためリンクにしない |
