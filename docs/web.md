...
| `OneMarkPredictionDiagram.astro` | 1 マーク予想（AI 寄与度ベース）の可視化。`aiEvaluation` は各予想者ごとの評価を採用するため予想者間で図が異なり得る（現状 B君予想は A君予想と同一 recipe のため一致）。PredictorCard 内に表示 |
...
| `DailyBetSummary.astro` | トップページの当日サマリー。締切済み全レースを集計した 3連単 戦略の的中率・回収率を予想者別（直前買い目のみ）に表示（A君直前 / B君直前 の 2 カード。`activePredictors()` を slot 昇順でループ。新スキーマ `prediction.predictions[]` を優先し、無い場合のみ primary 予想者を legacy `prediction.betPayout.realtime` でフォールバック） |
