...
| `PredictorCard.astro` | 1 予想者ぶんの予想カード。表示名・買い目 (BettingPicks) ・回収率 (BetPayoutSummary)・AI 評価チャートを 1 セクションに集約。レース詳細ページは `prediction.predictions[]` をループしてこれを縦並びレンダリングする。任意 prop `startPrediction` / `oneMarkAiEvaluation` が両方渡されたとき（本命予想=slot1 / モーター予想=slot4 のカード。過去日は退役済みの slot2/3 も対象）スタート予想・1マーク予想の 2 図をカード内に 2 カラムグリッドで小さく横並び表示する。`startNote` prop は `StartPredictionDiagram` の `stNote` へ転送。任意 prop `recipeNote` を渡すとカード見出し直下に注記を表示する（モーター予想カードは motor4 差し替えを説明する。過去日カードはモーター評価変更予想=motor2rate 差し替え / 展開予想=展開優位pt 追加を説明）。スタート予想は全予想者共通で AI 推定 ST（帯つき）、1マークは予想者ごとの AI 評価を渡す（モーター予想は本命予想と recipe が異なるため図は異なり得る） |
| `BettingPicks.astro` | 当日買い目・直前買い目の三連単フォーメーションと的中可否 (PredictorCard 内部で利用)。`predictorName` prop が渡されると見出しに「{予想者名}当日買い目」「{予想者名}直前買い目」として予想者名をプレフィクスする。`predictorId` prop で買い目しきい値（`bettingToleranceFor`。距離基準はオーバーライド無しで ±0.10、強さpt 基準の v8_aionly は ±5.0pt）と走行距離の予測 ST 種別（`oneMarkDistanceOptionsFor`。`useEstimatedST` な予想者 v5_slit / v7_aggregate / v8_aionly は AI 推定 ST、他は全国平均 ST）と買い目候補の選定基準（`bettingBasisFor`。`strengthOnlyBetting` な v8_aionly のみ走行距離ではなく強さpt）を予想者ごとに切替。どちらもバッチの買い目生成（`prediction-builder.ts`）と同じヘルパーで解決するため、表示される買い目と的中・回収率の集計対象になる買い目が一致する。説明文のしきい値表記・予測 ST 種別も実値に追従する |
