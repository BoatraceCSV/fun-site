...
| `RacePreview` / `RacePreviewBoat` / `RaceWeather` | 直前情報。`RacePrediction.preview` にぶら下がり、tkz（体重・展示タイム・チルト、`exhibitionTime` は 0→null）と sui（水面気象、天候はコード生値）を結合。どちらの CSV も未取得のレースでは `preview` 自体が undefined |
...
| `BettingTolerance` / `bettingToleranceFor` | 買い目の着順別しきい値（1着 / 2着 / 3着 の ± 許容幅）と予想者 ID からの解決関数。既定は全着順 ±0.10（`DEFAULT_BETTING_TOLERANCE`）。`BETTING_TOLERANCE_BY_PREDICTOR` に予想者別オーバーライドを定義（現状 `v2_tenkai`=B君予想のみ 1着0.02 / 2着0.10 / 3着0.20）。導出根拠は boatracecsv 側 [`notebooks/threshold_optimization.ipynb`](https://github.com/BoatraceCSV) |
