...
- **買い目生成**: 1 マーク予想の走行距離（`computeOneMarkDistances`）から各着順の上位艇 ± しきい値以内を候補にして三連単フォーメーションを生成（当日買い目・直前買い目の 2 種類）。しきい値は予想者ごとに `bettingToleranceFor(predictor.id)` で解決する（現状オーバーライド無しで全予想者 ±0.10）。走行距離の予測 ST 種別も予想者ごとに `oneMarkDistanceOptionsFor(predictor.id)` で解決する（レジストリの `PredictorSpec.useEstimatedST` が唯一の情報源。web の `BettingPicks.astro` が同じヘルパーを呼ぶことで、画面に表示される買い目と的中・回収率の集計対象になる買い目が一致する）。各着のしきい値窓を取った後、有効な出目（1-2-3 着が相異なる組合せ）に 1 つも使われないデッド候補は各着候補から除外する（着順別しきい値を設定した場合に 1着の本命艇が広い 3着窓に重複表示される事象を防ぐ。除外対象はどの出目にも使えない艇のみのため組合せ数・回収率は不変）
