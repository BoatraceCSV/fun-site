...
- **買い目生成**: 1 マーク予想の AI 寄与度から三連単フォーメーションを生成（A君直前買い目・B君直前買い目の 2 種類。内部キーは `daily` / `realtime` で、それぞれ朝バッチ時点 / 直前情報反映後の AI 評価に対応）
- **的中判定**: `results/realtime` が取得できているレースは [`packages/shared/src/utils/bet-hit.ts`](../packages/shared/src/utils/bet-hit.ts) で A君直前・B君直前買い目それぞれの的中可否を判定
- **3連単 払戻 / 回収率**: `results/payouts` が取得できているレースは [`packages/shared/src/utils/bet-payout.ts`](../packages/shared/src/utils/bet-payout.ts) でフォーメーション内の組合せ数 × ¥100 を賭け金、的中時は `sanrentan.payout` を払戻として `RaceBetPayoutSummary` を計算（A君直前 / B君直前 別）
...
| `series-aggregator.ts` | 節集計 (B君直前買い目戦略の的中率・回収率)。会場ページの「今節成績」セクション用。`SERIES_LOOKBACK_DAYS = 7`。`_meta/series-summary.json` を `packages/web/src/data/_meta/` に書き出す |
