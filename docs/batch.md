...
| `index.ts` | `fetchAllCsvData()` で固定 CSV（title / race_cards / stt / tkz / sui / results / payouts）+ 各 active 予想者の index を並列取得して `FetchedCsvData` に統合 (`indexesByPredictor: PredictorIndexFetch[]` で予想者単位に分離)。tkz / sui は `RacePrediction.preview` に結合される |
