...
event-parser  ─►  build-state check  ─►  fetcher  ─►  prediction-builder  ─►  site-builder
                  (早期 return 判定)      (CSV 5 種)    (RacePrediction 生成)    (write → dates-index → astro build → deploy → dates-index 書き戻し)
```
...
| `index.ts` | `fetchAllCsvData()` で 6 種を並列取得して統合 |
...
レースコードで 6 種の CSV を結合し、レース 1 件あたり `RacePrediction` を組み立てる。
