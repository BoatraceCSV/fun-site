...
- **予測 ST の決定**: `estimate/racer_st` の AI 推定 ST（実測 ST 履歴ベース。boatracecsv `build_racer_st.py` が日次生成）を `RaceRacer.estimatedST` に取り込む。使用するのは **`useEstimatedST` な予想者** （`PredictorSpec.useEstimatedST`。`computeOneMarkDistances` の opt-in 引数と `startPredictionEstimated` で分離。`strengthOnlyBetting` を併せ持つ予想者では買い目が強さpt 基準になるため表示にのみ効く）。それ以外の予想者と CSV 未生成時は従来どおり全国平均 ST。なお現行 active な予想者に `useEstimatedST` を持つものは無く、この分岐が効くのは過去日の退役予想者ぶんのみ。ただしスタート予想図は予想者に依らず推定 ST 版 (`startPredictionEstimated`) を使うため、`racer_st` の取り込み自体は常に必要
- **stt 未取得時のフォールバック**: 進入コース = 枠番、スタートタイミング = 全国平均ST（`useEstimatedST` な予想者の推定 ST 版スタート予想は AI 推定 ST）
...
- **買い目生成**: 1 マーク予想の走行距離（`computeOneMarkDistances`）から各着順の上位艇 ± しきい値以内を候補にして三連単フォーメーションを生成（当日買い目・直前買い目の 2 種類）。しきい値は予想者ごとに `bettingToleranceFor(predictor.id)` で解決する（距離基準の予想者はオーバーライド無しで ±0.10。強さpt 基準 = `strengthOnlyBetting` な予想者は ±5.0pt。予想者別のしきい値オーバーライド `BETTING_TOLERANCE_BY_PREDICTOR` は現在空）。走行距離の予測 ST 種別も予想者ごとに `oneMarkDistanceOptionsFor(predictor.id)` で解決する（レジストリの `PredictorSpec.useEstimatedST` が唯一の情報源。web の `BettingPicks.astro` が同じヘルパーを呼ぶことで、画面に表示される買い目と的中・回収率の集計対象になる買い目が一致する）。買い目候補の選定基準も予想者ごとに `bettingBasisFor(predictor.id)` で解決する（`PredictorSpec.strengthOnlyBetting` が唯一の情報源。該当する予想者は走行距離ではなく **強さpt のみ ±5.0pt 窓** で候補を選定し、予測 ST は買い目に影響しない。`useEstimatedST` と同じく現行 active な予想者は該当しない）。各着のしきい値窓を取った後、有効な出目（1-2-3 着が相異なる組合せ）に 1 つも使われないデッド候補は各着候補から除外する（着順別しきい値を設定した場合に 1着の本命艇が広い 3着窓に重複表示される事象を防ぐ。除外対象はどの出目にも使えない艇のみのため組合せ数・回収率は不変）
...
- **母数の判定 (確定済み)**: 的中率・回収率の集計母数は結果が確定したレース （`isSettledResult` = 1〜3 着が揃う）に限定する。[`packages/shared/src/utils/race-result.ts`](../packages/shared/src/utils/race-result.ts) の `isSettledResult` を集計入口 (predictor-stats / predictor-breakdown / series-state-store / web `DailyBetSummary`) で評価し、未確定レースを母数・購入額から除外する。中止・不成立・返還レースは着順が出ないため恒久的に除外される（返還金は購入額を計上しない扱い）
