...
/** 値が小さいほど上位の競技順位（1,1,3 形式）。null / 非有限値は順位を付けない */
const competitionRanks = (values: readonly (number | null)[]): (number | null)[] => {
  const sorted = [
 ...
 */
const spearman = (xs: readonly number[], ys: readonly number[]): number | null => {
