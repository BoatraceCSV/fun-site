...
export const runPipeline = async (): Promise<void> => {
  ...
  const indexSummary = csvData.indexesByPredictor
    ...
    .join(",");
  console.info(
    `Fetched: ${csvData.titles.length} titles, ${csvData.raceCards.length} race_cards, ${csvData.stt.length} stt, ${csvData.tkz.length} tkz, ${csvData.sui.length} sui, ${csvData.originalExhibition.length} orig_exhibition, ${csvData.tokutenHayami.length} tokuten_hayami, ${csvData.recentNational.length} recent_national, ${csvData.recentLocal.length} recent_local, ${csvData.waku10.length} waku10, ${csvData.motorStats.length} motor_stats, ${csvData.racerSt.length} racer_st, ${csvData.suji.length} suji, ${csvData.kimaritePicks.length} kimarite_picks, ${csvData.kimarite.length} kimarite, index[${indexSummary}], ${csvData.results.length} results, ${csvData.payouts.length} payouts`,
  );
